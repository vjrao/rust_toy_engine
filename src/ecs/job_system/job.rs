...
thread_local!(pub static JOB_POOL: Pool = Pool::new());
...
impl Job {
	...
	pub fn new<F, R: Send>(f: F, dest: *mut JobResult<R>) -> Job where F: FnMut(&Worker) -> R {
	...
	pub unsafe fn call(self, args: &Worker) {
...
impl Pool {
	...
	pub fn alloc(&self, job: Job) -> *mut Job {		
		unsafe {
