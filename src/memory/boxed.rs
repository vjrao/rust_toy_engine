...
pub struct AllocBox<T: ?Sized, A = DefaultAllocator>
    where A: Allocator
{
    ptr: Unique<T>,
    alloc: A,
...
impl<T, A: Allocator> AllocBox<T, A> {
    ...
    pub fn in_alloc(x: T, alloc: A) -> Self {
        ...
        let addr: Address = match Kind::for_value(&x) {
            ...
            None => unsafe { NonZero::new(heap::EMPTY as *mut u8) },
        };
...
impl<T, A> Clone for AllocBox<T, A>
    where T: Clone,
          A: Allocator + Clone
{
    fn clone(&self) -> Self {
...
impl<Args, Alloc, F> FnBox<Args, Alloc> for F
...
impl<Alloc, Args, F> FnOnce<Args> for AllocBox<F, Alloc>
    where Alloc: Allocator,
          F: FnOnce<Args>
{
    type Output = F::Output;
...
mod tests {
    ...
    fn destructors() {
        ...
        let mut i = 0;
        {
            let _ = AllocBox::new(Incrementor { i: &mut i });
        }
